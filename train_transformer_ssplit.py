...
for epoch in tqdm(range(epochs)):
    ...
    with torch.no_grad():
        for X,y,x_d,y_d in dataloader_test:
            ...
            writer.add_images('Ground_truth', np.expand_dims(y.detach().cpu().numpy()[0],axis=1), 0)
            writer.add_images('Predicts', np.expand_dims(outputs.detach().cpu().numpy()[0],axis=1), 0)
            #img+=0.005
